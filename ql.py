...
class QPlayer(Player):
    ...
    # y[0,22]+1 x[0,15] p[0,15] t[0,10]
    @classmethod
    def flatten_grid_count(cls, y, x, p, dx, dy):
        x //= 32
        x = min(x, 15)
        p //= 32
        if dy <= 0:
    ...
    def state(self, table):
        ...
        if y > 22:
            return 1
        return self.flatten_grid_count(y, table.ball.x,
            table.paddle.centerx, table.ball.dx, table.ball.dy)
    ...
    def strategy_train(self, table):
        ...
        if s >= 64769:
            print('state', s)
            ball = table.ball
            print('ball', ball.x, ball.y, ball.dx, ball.dy, ball.dt)
            print('paddle', table.paddle.centerx)
            exit()
